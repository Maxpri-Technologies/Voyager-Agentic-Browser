...
async function getModelDecision(dataUrl, didLastActionFail = false, interactiveElements = [], learnedLessons = [], userMemories = []) {
  ...
  const systemInstruction = `
    ...
    For a request to get, buy, purchase, or add multiple things to an online cart (including a follow-up that says to get items from a previously generated list), the required outcome is execution, not a recommendation list. Extract the individual product names from the user's request, chat, task board, or working memory and process them one at a time. Search for exactly one product per search; never paste a list of products into one search box. For each product, select a matching result, choose required variants only when unambiguous, click the product's Add to Cart control, and inspect the page or cart confirmation to verify it was added before starting the next product. Keep the current product and remaining products in memory. Do not issue stop while any requested product is not verified in the cart. Do not proceed to checkout or payment unless the user explicitly asks.
    ...
    CRITICAL Rules:
    - When the user says to get items, make sure to add them to the cart and verify they are in the cart before moving on. Do not stop until all requested items are confirmed in the cart.
